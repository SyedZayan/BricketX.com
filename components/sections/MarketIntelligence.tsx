...
// --- MOCK DATA FOR REAL ESTATE (CMS Managed Later) ---
...
export default function MarketIntelligence() {
  ...
  // State to hold our live Gold Data
  const [goldData, setGoldData] = useState({
    price: 2162.40, // Fallback dummy data
    priceGram: 69.45,
    chp: 2.3, // Change percentage
    isLoading: true
  });
    ...
    const fetchGoldData = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // 1 Troy Ounce = 31.1035 grams. We calculate the gram price here.
        ...
        console.error("Failed to load live gold data, using fallbacks.");
        setGoldData(prev => ({ ...prev, isLoading: false }));
      }
    };
    ...
    <section className="w-full bg-bricket-white py-24 lg:py-32 px-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          {/* =========================================
              LEFT COLUMN: GOLD MARKET TERMINAL 
          ========================================= */}
          <div className="lg:col-span-7 bg-bricket-dark rounded-2xl shadow-xl border border-bricket-grey/20 p-8 flex flex-col relative overflow-hidden animate-fade-in-up [animation-delay:150ms]">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 relative z-10">
              <div>
                <h3 className="text-2xl font-bold text-bricket-white flex items-center gap-2">
                  <span className={`w-3 h-3 rounded-full ${goldData.isLoading ? 'bg-bricket-grey' : 'bg-bricket-gold animate-pulse'}`}></span>
                  Live Gold Market
                </h3>
                <p className="text-sm text-bricket-grey/70 mt-1">XAU/USD • Spot Price</p>
              </div>
              <div className="mt-4 sm:mt-0 text-left sm:text-right">
                <p className="text-xs font-bold text-bricket-white uppercase tracking-wider">Live API Sync</p>
                <p className="text-xs text-bricket-grey/70 mt-0.5">Updated Hourly</p>
              </div>
            </div>

            {/* Big Data Row (Wired to API) */}
            <div className={`grid grid-cols-2 sm:grid-cols-4 gap-6 mb-10 relative z-10 transition-opacity duration-500 ${goldData.isLoading ? 'opacity-50' : 'opacity-100'}`}>
              <div>
                <p className="text-[10px] text-bricket-grey/70 uppercase font-bold tracking-wider mb-1">Price (USD/Oz)</p>
                <p className="text-2xl sm:text-3xl font-extrabold text-bricket-white">
                  ${goldData.price.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </p>
              </div>
              <div>
                <p className="text-[10px] text-bricket-grey/70 uppercase font-bold tracking-wider mb-1">Price (Gram)</p>
                <p className="text-xl sm:text-2xl font-bold text-bricket-grey">
                  ${goldData.priceGram.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </p>
              </div>
              <div>
                <p className="text-[10px] text-bricket-grey/70 uppercase font-bold tracking-wider mb-1">Daily Trend</p>
                <p className={`text-xl font-bold flex items-center gap-1 ${goldData.chp >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                  <svg className={`w-5 h-5 ${goldData.chp < 0 ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" /></svg>
                  {goldData.chp > 0 ? '+' : ''}{goldData.chp.toFixed(2)}%
                </p>
              </div>
              <div>
                <p className="text-[10px] text-bricket-grey/70 uppercase font-bold tracking-wider mb-1">Market Sentiment</p>
                <p className="text-xl font-bold text-bricket-gold">
                  {goldData.chp >= 0 ? 'Bullish' : 'Bearish'}
                </p>
              </div>
            </div>
            ...
            <div className="flex-1 min-h-[200px] bg-bricket-white/5 border border-bricket-white/10 rounded-xl relative z-10 flex flex-col p-4">
              ...
              {/* Pseudo-Chart Outline */}
              <div className="flex-1 w-full h-full flex items-end justify-between gap-1 mt-4 relative opacity-70">
          ...
          {/* =========================================
              RIGHT COLUMN: REAL ESTATE MARKETS
          ========================================= */}
          <div className="lg:col-span-5 bg-[#fbf8f3] rounded-2xl shadow-xl border border-bricket-grey/10 p-8 flex flex-col animate-fade-in-up [animation-delay:300ms]">
            ...
            {/* Region Toggle */}
            <div className="flex p-1 bg-bricket-white rounded-lg mb-6 shadow-sm border border-bricket-grey/10">
              <button 
                onClick={() => setActiveRegion('uae')}
                className={`flex-1 py-2 text-sm font-bold rounded-md transition-all ${activeRegion === 'uae' ? 'bg-bricket-dark shadow-md text-bricket-white' : 'text-bricket-grey hover:text-bricket-dark'}`}
              >
                UAE Markets
              </button>
              <button 
                onClick={() => setActiveRegion('pk')}
                className={`flex-1 py-2 text-sm font-bold rounded-md transition-all ${activeRegion === 'pk' ? 'bg-bricket-dark shadow-md text-bricket-white' : 'text-bricket-grey hover:text-bricket-dark'}`}
              >
                Pakistan Markets
              </button>
            </div>
            ...
            <div className="flex flex-col gap-3">
              ...
              {realEstateData[activeRegion].map((item, index) => (
                <div key={index} className="grid grid-cols-12 items-center py-3 border-b border-bricket-grey/5 last:border-0 hover:bg-bricket-white transition-colors rounded-lg px-2 -mx-2 cursor-pointer group">
                  ...
                  <div className="col-span-2 flex justify-end">
                    {item.trend === 'up' ? (
                      ...
                      <div className="w-8 h-8 rounded-full bg-gray-200/50 flex items-center justify-center text-gray-500">
            ...
            <div className="mt-auto pt-6">
              <button className="w-full py-3 border border-bricket-grey/20 rounded-lg text-sm font-bold text-bricket-dark hover:border-bricket-dark transition-colors bg-bricket-white">
